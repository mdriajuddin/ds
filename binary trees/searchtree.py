...
class BSTMap:
    ...
    def valueOf(self, key):
        node = self._bstSeatch(self._root, key)
    ...
    def _bstInsert(self, subtree, key, value):
        if subtree is None:
            ...
            subtree.right = self._bstInsert(subtree)   
    ...
    # Helper method tha tremoves an existing item recursively.
...
class _BSTMapIterator:
    def __init__(self, root, size):
        ...
        self._curItem = 0 # Resrt the current item index.

    def __iter__(self):
        return self
    
    # Returns the next key form the array of keys
    def __next__(self):
...
class _BSTMaoIterator:
    def __init__(self, root):
        # Create a stack for user in traversing the  tree.
        self._theStack = Stack()
        # We must traverse down to the node containing the smallest key
        # during whick each node along the path is pushed onto the stack.
    ...
    def __next__(self):
        ...
        if self._theStack.isEmpty():
            ...
            key = node.key
            # If this node has a subtree rooted as the right child,
            # we must find the node in that subtree that contains the smallest key.
    ...
    def _traveseToMinNode(self, subtree):
        if subtree is not None:
            self._theStack.push(subtree)
            self._traveseToMinNode(subtree.left)

...
class _23TreeNode(object):
    ...
    def hasKey(self,target):
        if (target == self.key1) or (self.key2 is not None \
            and target == self.key2):
            return True
        else:
            return False
    ...
    # Chooses the appropriate brach for the given target.
    ...
    def _23Search(subtree, target):
        # If we encounter a null pointer, the target is notn in the tree.
        if subtree is None:
            return None
    ...
    def _23Insert(self, key, newitem):
        # If the is empty, a node has to be created for the first key.
    ...
    def _23RecInsert(self,subtree, key, newitem):
        ...
        # Otherwise, it's an interior node.
            ...
            # If th child was split, the promoted key an dreference have to be
...
class Tree23Map:
    ...
    # node N created in the function will also be an interior node.
    # In that case, the links of the interior node have to be set appropriately.
    def _23SplitNode(self, node, key, data, pRef):
        # Craete the new node, the reference to which will be promoted.

...
class Map:
    ...
    # Determines if the mao contains the given key.
    def __contains__(self, key):
        ndx = self._findPosition(key)
        return ndx is not None
    
    # Adds a new entry to the map if the key doe3s exist. Otherwiese,
    # the new values replaces the current value associated with the key.

    def add(self, keu, value):
        ndx = self._findPosition(key)
        if ndx is not None:   # if the key was found
            self._entryList[ndx].value = value
            return False
        else: # Othewise add an new entry
    ...
    # Helper method usef to dind the index position of a category. 
    # If the key is not found, None if returnd.
    def _findPosition(self, key):
        # Iterate through each entry in the list.
        for i in range(len(self)):
            # Is the key stores in the ith entry?
